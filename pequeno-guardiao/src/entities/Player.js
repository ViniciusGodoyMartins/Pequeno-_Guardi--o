...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  update(t, d) {
    const l = this.keys.A.isDown || this.cursors.left.isDown || this.scene.touchLeft;
    const r = this.keys.D.isDown || this.cursors.right.isDown || this.scene.touchRight;
    ...
    if (
      ...
      this.scene.touchJumpQueued
    ) {
      this.jumpQueued = t;
      this.scene.touchJumpQueued = false;
    }
  ...
  updateWeapon() {
    ...
    const angle = this.scene.aimAngle ? this.scene.aimAngle() : 0;
