...
export class PlayScene extends Phaser.Scene {
  ...
  commonSetup() {
    ...
    // Botão esquerdo atira; botão direito fica desativado (sem menu de contexto e sem ação)
  ...
  makeHud() {
    ...
    this.msg = this.add.text(640, 655, '', {
  ...
  createNormalLevel() {
    ...
    [
      [500, 680, 1000, 80], [1240, 680, 400, 80], [1800, 680, 600, 80], [2300, 680, 400, 80],
      [350, 530, 180, 22], [760, 470, 200, 22], [1180, 535, 190, 22], [1580, 455, 200, 22], [2050, 525, 200, 22]
    ].forEach(v => {
    ...
    // (Antes esse piso ficava mais abaixo, então o inimigo caía dentro do
    // buraco e ficava preso debaixo da plataforma sólida, sem conseguir
    // subir de volta.)
    ...
    this.level.enemies.forEach((k, i) => this.spawnEnemy(420 + i * 330, 450 + (i % 2) * 130, k, true));

    this.terminal = this.physics.add.staticSprite(1780, 600, 'terminal');
    this.prompt = this.add.text(1780, 525, '', {
      fontFamily: 'monospace', fontSize: '18px', fontStyle: 'bold', color: '#fff', backgroundColor: '#071426',
      padding: { x: 10, y: 6 }
    }).setOrigin(0.5);
    this.item = this.physics.add.staticSprite(1180, 500, 'item');
  }
  ...
  update(t, d) {
    ...
    if (this.terminal) {
      ...
      this.prompt.setText(!this.terminalOn && dist <= 155 ? (this.remainingThreats === 0 ? 'PRESSIONE E' : `${this.remainingThreats} AMEAÇA(S) RESTANTE(S)`) : '');
    }
  ...
  fire(pointer) {
    ...
    const baseA = Phaser.Math.Angle.Between(this.player.x, this.player.y - 10, q.x, q.y);
